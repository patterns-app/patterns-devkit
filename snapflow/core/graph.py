...
#             key_prefix = n.key + "."
#             stdout_key = key_prefix + n.get_stdout_key()
#             stdout_lookup[n.key] = stdout_key
#             if n.nodes:
#                 sub_nodes = []
#                 for sub_node in n.nodes:
#                     new_sub_node = update(sub_node, key=key_prefix + sub_node.key)
#                     sub_nodes.append(new_sub_node)
#                 new_node = update(n, nodes=sub_nodes)
#                 flattened_nodes.append(new_node)
#         for n in config.nodes:
#             for name, input_key in n.inputs.items():
#                 n.inputs[name] = stdout_lookup.get(input_key, input_key)
#         update(config, nodes=flattened_nodes)
#     return config


# class Graph:
#     def __init__(self, env: Environment, config: GraphCfg):
#         self.env = env
#         self._original_config = config
#         self._flattened_config = flatten_graph_config(config)
#         self._nodes: Dict[str, Node] = flatten_graph_config(config)
#         if nodes:
#             for n in nodes:
#                 self.add_node(n)
