...
def test_sql_snap_interface_fn():
    @Sql
    def ssnap():
        sql = """select 1 from from t1:T1
            join t2:Any on t1.a = t2.b left join t3:T2
...
def test_sql_snap_interface_fn_no_autodetect():
...
def test_sql_snap_interface_fn_snap():
    @SqlSnap
    def ssnap():
        sql = """select 1 from from t1:T1
            join t2:Any on t1.a = t2.b left join t3:T2
    ...
    @SqlSnap(autodetect_inputs=False, name="newname")
    def snp():
        sql = """select 1 from from t1:T1
            join t2 on t1.a = t2.b left join t3:T2
            on"""
...
# def test_sql_snap_interface_table_alias():
#     sql = """select:DataBlock[T]
#         1
#         from -- comment inbetween
#         input as
