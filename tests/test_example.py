...
from dags import DataBlock, pipe, DataSet
...
def test_incremental():
    ...
    env.add_module(core)
    N = 2 * 4
    env.add_node("source", customer_source, config={"total_records": N})
    env.add_node("metrics", shape_metrics, inputs="source")
    output = env.produce("metrics", target_storage=s)
