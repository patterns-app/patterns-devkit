...
class _NodeFuncFinder(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        name = node.name
        count = sum(
            1 for it in node.decorator_list if _is_basis_node_decorator(it)
        )
        if count == 0:
            return
        if count > 1:
            raise ValueError(f'@node decorator cannot be used more than once on a function')
        self.found += 1

        args = node.args
        if args.kwarg or args.vararg or args.posonlyargs or args.kwonlyargs:
            raise ValueError(f'Node function {name} cannot use varargs or keyword-only args')
        if len(args.args) != len(args.defaults):
            raise ValueError(f'Node function {name} must specify an input, output, or parameter for all arguments.')
    ...
    def _consume_call(self, name: str, call: _Call):
        ...
        if call.name.startswith('Input'):
            ...
            self.o.append(OutputDefinition(
                port_type=port_type,
                name=name,
                description=get('description', str),
                schema_or_name=get('schema', str),
            ...
            self.p.append(ParameterDefinition(
                name=name,
            ...
            raise ValueError(f'Node functions must declare inputs, outputs, or parameters: {call.name}')
        if call.kwargs:
            raise ValueError(f'got an unexpected keyword argument {call.kwargs.popitem()[0]}')
...
def _parse_call(node) -> _Call:
    ...
    if not isinstance(node, ast.Call):
        raise ValueError(f'Node functions must declare inputs, outputs, or parameters: {node}')
    if node.args:
        raise ValueError(f'Node functions only accept keyword arguments')
    return _Call(name=_get_qualified_name(node.func), kwargs={
        a.arg: _parse_port_arg(a.value) for a in node.keywords
    })
