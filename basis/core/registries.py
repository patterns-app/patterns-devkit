...
from basis.utils.registry import Registry, RegistryError, T, UriRegistry
...
class ComponentRegistry(UriRegistry):
    def __init__(
        ...
        self._module = module
        self._module_key = self._module.key if self._module else DEFAULT_MODULE_KEY
...
class DataFunctionRegistry(ComponentRegistry):
    ...
    def process(self, df_like: Union[DataFunctionLike, str]) -> DataFunctionSet:
        from basis.core.data_function import (
            DataFunction,
            ensure_datafunction,
            DataFunctionSet,
        ...
        if isinstance(df_like, DataFunctionSet):
            dfs = df_like
        else:
            df = None
            if isinstance(df_like, DataFunction):
                ...
                df = ensure_datafunction(df_like, module_key=self._module_key)
            elif isinstance(df, str) and df.endswith(".sql"):
                if not self._module or not self._module.module_path:
                    raise Exception("Module path not set")
                sql_file_path = os.path.join(self._module.module_path, df)
            ...
            dfs = DataFunctionSet(
                key=df.key, module_key=df.module_key, version=df.version
            )
            dfs.add(df)
        return dfs
