...
def add_persisting_sdb_listener(session_maker: sessionmaker):
    ...
    def intercept_t2p(session, instance):
        ...
        if isinstance(instance, StoredDataBlockMetadata):
            printd(
                f"Persisted StoredDataBlock SDR#{cf.bold(instance.id)} DR#{cf.bold(instance.data_block.id)} {cf.magenta(instance.data_block.expected_otype_uri)} {cf.dimmed_magenta(instance.data_format)}"
            )
