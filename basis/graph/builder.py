...
class _GraphBuilder:
    ...
    def _parse_graph_cfg(
        ...
        nodes = [
            self._parse_node_entry(node, node_dir, parents, outputs_by_name)
    ...
    def _apply_edges(self, node: ConfiguredNode, outputs_by_name: _OutputsByName):
        ...
        names.update(
            {
                e.output_port: e.input_port
                for e in node.declared_edges
                if e.output_port in names
            }
        )
    ...
    def _track_outputs(
        ...
        names.update(
            {c.src: c.dst for c in (mapping or [])}
        )  # override names from yaml
    ...
    def _parse_node_entry(
        ...
        self._track_outputs(
            configured_node, interface.outputs, node.outputs, outputs_by_name
        )
        return configured_node

    def _parse_node_interface(
        self, node_file_path: Path, parents: List[str]
    ) -> Tuple[NodeInterface, NodeType]:
