...
DAGS `pipes` optionally expose a type interface, called an `ObjectType`, that describes the
 structure and semantics of the expected input and output data. These type interfaces
 allow the DAGS community to build an ecosystem of components that anyone can reuse project to
...
### Features:
 ...
 - **Reusable modules and components**  
   There are hundreds of `pipes`, types, and external data `Sources` and `Targets` ready to
    snap together in the DAGS Repository [Coming soon].
...
NB: DAGS is 0.1.1 **ALPHA** software. Expect breaking changes to core APIs. 
...
```python
from dags import current_env
from dags_modules import stripe, bi
